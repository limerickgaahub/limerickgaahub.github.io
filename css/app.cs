...
#modal-standings tbody tr:hover td { background:#f1f9f4; }
...
#modal-standings th:nth-child(6), #modal-standings td:nth-child(6){ min-width:5.4rem; white-space:nowrap; } /* PF */
#modal-standings th:nth-child(7), #modal-standings td:nth-child(7){ min-width:5.4rem; white-space:nowrap; } /* PA */
#modal-standings th:nth-child(8), #modal-standings td:nth-child(8){ min-width:5.4rem; white-space:nowrap; } /* Diff */
#modal-standings th:nth-child(9), #modal-standings td:nth-child(9){ min-width:5.2rem; white-space:nowrap; } /* Pts */
...
/* Standings modal: compact layout for mobile (expanded view) */
#standings-modal .table-wrap{ border-radius:12px; }

#modal-standings thead th,
#modal-standings tbody td{
  padding:10px 8px;     /* tighter than global */
  font-size:14px;
}

/* Team can wrap; numbers stay compact */
#modal-standings thead th:first-child,
#modal-standings tbody td:first-child{
  white-space:normal;   /* override header nowrap for Team */
  width:38%;
}
...
#modal-standings th.right,
#modal-standings td.right {
  min-width: 4.2em; /* base for P, W, D, L, PF, PA, Pts */
}
...
/* Center align numeric columns P → Pts in modal standings */
#modal-standings th:nth-child(n+2):nth-child(-n+9),
#modal-standings td:nth-child(n+2):nth-child(-n+9) {
  text-align: center !important;     /* override .right */
  display: table-cell;               /* keep table semantics */
  vertical-align: middle;            /* tidy vertical placement */
}




/* Modal standings striping (apply to TD so it always shows) */
#modal-standings tbody tr:nth-child(even) td { background:#fafafa; }
#modal-standings tbody tr:nth-child(odd)  td { background:#ffffff; }

/* Optional: stop global hover fighting the modal striping */
#modal-standings tbody tr:hover td { background: inherit; }

/* Modal: right-align numeric columns (P → Pts) */
#modal-standings th:nth-child(n+2):nth-child(-n+9),
#modal-standings td:nth-child(n+2):nth-child(-n+9){
